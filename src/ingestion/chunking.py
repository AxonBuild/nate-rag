"""Hierarchical chunking pipeline — identical strategy to tom-orent-RAG."""
import re
from typing import List
from src.ingestion.config import settings
from src.ingestion.models import Document, ChunkMetadata
...
class HierarchicalChunker:
    ...
    def _split_by_words_at_sentence_boundary(text: str, chunk_size_words: int) -> List[str]:
        """Split at sentence boundaries after accumulating at least chunk_size_words."""
    ...
    def chunk_document(
        ...
        """Chunk document into 3 hierarchical levels, attaching topic and doc_type."""
    ...
    def _create_page_chunks(
        self, document: Document, document_id: str, topic, doc_type
    ) -> List[tuple[Document, ChunkMetadata]]:
        texts = self._split_by_words_at_sentence_boundary(document.text, settings.chunk_size_page)
        chunks = []
        for i, text in enumerate(texts):
            ...
            chunks.append((Document(text=text, metadata={**document.metadata, **meta.to_dict()}), meta))
        return chunks

    def _create_paragraph_chunks(
        self, page_chunks: List[tuple[Document, ChunkMetadata]], document_id: str, topic, doc_type
    ) -> List[tuple[Document, ChunkMetadata]]:
        chunks = []
        for page_doc, page_meta in page_chunks:
            texts = self._split_by_words_at_sentence_boundary(page_doc.text, settings.chunk_size_paragraph)
            for i, text in enumerate(texts):
                ...
                chunks.append((Document(text=text, metadata={**page_doc.metadata, **meta.to_dict()}), meta))
        return chunks

    def _create_content_chunks(
        self, paragraph_chunks: List[tuple[Document, ChunkMetadata]], document_id: str, topic, doc_type
    ) -> List[tuple[Document, ChunkMetadata]]:
        chunks = []
        for para_doc, para_meta in paragraph_chunks:
            texts = self._split_by_words_at_sentence_boundary(para_doc.text, settings.chunk_size_content)
            for i, text in enumerate(texts):
                ...
                chunks.append((Document(text=text, metadata={**para_doc.metadata, **meta.to_dict()}), meta))
